...
const igraci = [
    ...
    { id: 5, ime: "Igrač 5", boja: "orange", simbol: "🟠", novac: 1500, pozicija: 0, polja: [] },
];
...
function osveziInformacijeIgraca() {
    igraci.forEach(igrac => {
        ...
        if (igracInfo) {
            const novacSpan = igracInfo.querySelector('.novac');
            const poljaSpan = igracInfo.querySelector('.polja');
            if (novacSpan) novacSpan.textContent = igrac.novac;
            if (poljaSpan) poljaSpan.textContent = igrac.polja.join(', ');
        }
    });
}
